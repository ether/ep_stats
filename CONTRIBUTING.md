# Contributing to ep_stats

Thanks for helping improve `ep_stats` — a plugin for [Etherpad](https://etherpad.org/). LLM/Agent contributions are explicitly welcome, provided they follow the rules below.
...
`ep_stats` is a plugin, so you develop it inside a checkout of [ether/etherpad](https://github.com/ether/etherpad).
...
# Install ep_stats from your local clone
pnpm run plugins i --path ../ep_stats

# Start the dev server (port 9001)
pnpm --filter ep_etherpad-lite run dev
```

Edits in `../ep_stats/` are picked up after a server restart.
...
* Etherpad version, ep_stats version, browser + OS
* Steps to reproduce
